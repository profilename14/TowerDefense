...
end end function swap_menu_context(e)get_active_menu().enable,get_menu(e).enable=false,true end function longest_menu_str(t)local e=0for n in all(t)do e=max(e,#n.text)end return e end function get_tower_data_for_menu()local e={}for n,t in pairs(global_table_data.tower_templates)do add(e,{text=t.name,color=t.text_color,callback=choose_tower,args={n}})end return e end function get_map_data_for_menu()local e={}for t,n in pairs(global_table_data.map_data)do add(e,{text=n.name,color={7,0},callback=load_map,args={t}})end return e end function parse_menu_content(t)if type(t)=="table"then local n={}for e in all(t)do add(n,{text=e.text,color=e.color or{7,0},callback=_ENV[e.callback],args=e.args})end return n end return _ENV[t]()end function toggle_mode()manifest_mode,sell=not manifest_mode sell_mode=not sell_mode end function swap_to_credits()game_state="credits"end function new_game()reset_game()game_state="map"swap_menu_context"map"end function load_map(e,t,n)pal()manifest_mode,auto_start_wave=true wave_round=t or 0freeplay_rounds=n or 0loaded_map,cur_level=e,e pathing=parse_path()for e=1,3do add(incoming_hint,Animator:new(global_table_data.animation_data.incoming_hint,true))end for e=0,15do grid[e]={}for t=0,15do grid[e][t]="empty"
