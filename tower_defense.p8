...
version 39
__lua__
function choose_tower(e)selected_menu_tower_id,get_active_menu().enable,shop_enable=e end function display_tower_info(t,i,o)local n,e=i+Vec:new(-1,-31),global_table_data.tower_templates[t]local t={{text=e.name,color=o},{text=e.prefix..": "..e.damage,color={7,0}},{text="cost: "..e.cost,color={coins>=e.cost and 3or 8,0}}}local i=longest_menu_str(t)*5+4tower_stats_background_rect=BorderRect:new(n,Vec:new(i+20,27),8,5,2)BorderRect.draw(tower_stats_background_rect)for e,i in pairs(t)do print_with_outline(i.text,combine_and_unpack({Vec.unpack(n+Vec:new(4,e==1and 2or 7*e))},i.color))end spr(e.icon_data,combine_and_unpack({Vec.unpack(tower_stats_background_rect.position+Vec:new(i,6))},{2,2}))end function display_tower_rotation(e,t)local e,n=global_table_data.tower_templates[selected_menu_tower_id],t+Vec:new(0,-28)tower_rotation_background_rect=BorderRect:new(n,Vec:new(24,24),8,5,2)BorderRect.draw(tower_rotation_background_rect)local t=n+Vec:new(4,4)if e.disable_icon_rotation then spr(e.icon_data,combine_and_unpack({Vec.unpack(t)},{2,2}))else draw_sprite_rotated(global_table_data.tower_icon_background,n,24,parse_direction(direction))draw_sprite_rotated(e.icon_data,t,16,parse_direction(direction))end end function rotate_clockwise()direction=Vec:new(-direction.y,direction.x)end function start_round()
...
end end function swap_menu_context(e)get_active_menu().enable,get_menu(e).enable=false,true end function longest_menu_str(n)local e=0for t in all(n)do e=max(e,#t.text)end return e end function get_tower_data_for_menu()local e={}for t,n in pairs(global_table_data.tower_templates)do add(e,{text=n.name,color=n.text_color,callback=choose_tower,args={t}})end return e end function get_map_data_for_menu()local e={}for n,t in pairs(global_table_data.map_data)do add(e,{text=t.name,color={7,0},callback=load_map,args={n}})end return e end function parse_menu_content(n)if type(n)=="table"then local t={}for e in all(n)do add(t,{text=e.text,color=e.color or{7,0},callback=_ENV[e.callback],args=e.args})end return t end return _ENV[n]()end function toggle_mode()manifest_mode,sell=not manifest_mode sell_mode=not sell_mode end function swap_to_credits()game_state="credits"end function new_game()reset_game()game_state="map"swap_menu_context"map"end function load_map(e,n,t)pal()manifest_mode,auto_start_wave=true wave_round=n or 0freeplay_rounds=t or 0loaded_map,cur_level=e,e pathing=parse_path()for e=1,3do add(incoming_hint,Animator:new(global_table_data.animation_data.incoming_hint,true))end for e=0,15do grid[e]={}for n=0,15do grid[e][n]="empty"
...
del(enemies,e)end end function parse_path(n)local e=n or global_table_data.map_data[loaded_map]local n,i,a=Vec:new(e.mget_shift),Vec:new(e.enemy_spawn_location),{}for t=0,15do for i=0,15do local e=Vec:new(i,t)+n if check_tile_flag_at(e,global_table_data.map_meta_data.path_flag_id)then add(a,e)end end end local r,t,c={},Vec:new(e.movement_direction),Vec:new(e.enemy_end_location)+n local e=i+n+t while e~=c do local l,d,f,s=Vec:new(e.x,e.y-1),Vec:new(e.x,e.y+1),Vec:new(e.x-1,e.y),Vec:new(e.x+1,e.y)local i,o=false if t.x==1then i,o=check_direction(s,{l,d},a,r,n)elseif t.x==-1then i,o=check_direction(f,{l,d},a,r,n)elseif t.y==1then i,o=check_direction(d,{f,s},a,r,n)elseif t.y==-1then i,o=check_direction(l,{f,s},a,r,n)end assert(i,"Failed to find path at: "..e.." in direction: "..t.." end: "..c)if i then t=normalize(o-e)e=o else end end return r end function check_direction(e,t,n,i,o)
if(e==nil)return
local a,r=is_in_table(e,n)if a then add(i,n[r]-o)else return check_direction(t[1],{t[2]},n,i,o)end return true,e end function spawn_enemy()while enemies_remaining>0do enemy_current_spawn_tick=(enemy_current_spawn_tick+1)%enemy_required_spawn_ticks
if(is_in_table(Vec:new(global_table_data.map_data[loaded_map].enemy_spawn_location),enemies,true))goto spawn_enemy_continue
if enemy_current_spawn_tick==0then local e=increase_enemy_health(global_table_data.enemy_templates[global_table_data[global_table_data.wave_set[cur_level]or"wave_data"][wave_round][enemies_remaining]])add(enemies,Enemy:new(global_table_data.map_data[loaded_map].enemy_spawn_location,unpack(e)))enemies_remaining-=1end::spawn_enemy_continue::yield()end end Tower={}function Tower:new(t,e,n)obj={position=t,dmg=e.damage,radius=e.radius,attack_delay=e.attack_delay,current_attack_ticks=0,cooldown=e.cooldown or 0,manifest_cooldown=-1,being_manifested=false,being_boosted=false,cost=e.cost,type=e.type,dir=n,rot=parse_direction(n),enable=true,animator=Animator:new(global_table_data.animation_data[e.animation_key],true)}add(animators,obj.animator)setmetatable(obj,self)self.__index=self return obj end function Tower:attack()
...
self.being_boosted=false if self.type=="tack"then Tower.apply_damage(self,Tower.nova_collision(self),self.dmg)elseif self.type=="floor"then local e={}add_enemy_at_to_table(self.position,e)foreach(e,function(e)e.burning_tick+=self.dmg end)elseif self.type=="sharp"then add(projectiles,Projectile:new(self.position,self.dir,self.rot,global_table_data.projectiles.rocket))elseif not self.being_manifested then if self.type=="rail"then Tower.apply_damage(self,raycast(self.position,self.radius,self.dir),self.dmg)elseif self.type=="frontal"then Tower.freeze_enemies(self,Tower.frontal_collision(self))end end end function Tower:nova_collision()local e={}nova_apply(self.radius,function(n)add_enemy_at_to_table(self.position+n,e)end)if#e>0then nova_apply(self.radius,function(e)add(particles,Particle:new(self.position+e,false,Animator:new(global_table_data.animation_data.blade,false)))end)end return e end function Tower:frontal_collision()local e,n={},{self.radius,self.dir}frontal_apply(n,function(n)add_enemy_at_to_table(self.position+n,e)end)if#e>0then frontal_apply(n,function(e)add(particles,Particle:new(self.position+e,false,Animator:new(global_table_data.animation_data.frost,false)))end)end return e end function Tower:apply_damage(n,o)local e=self.type for t in all(n)do if t.hp>0then local n,i=t.type,o if e=="tack"and n==7or e=="rail"and n==14then i=o\2elseif e=="rail"and n==7or e=="tack"and n==15then i*=2end t.hp-=i end end end function Tower:freeze_enemies(n)for e in all(n)do if not e.is_frozen then e.is_frozen,e.frozen_tick=true,self.dmg end end end function Tower:draw()
if(not self.enable)return
local e,n,t=self.position*8,Animator.get_sprite(self.animator),(self.type=="sharp"or self.type=="clock")and self.rot or parse_direction(self.dir)
if(self.being_boosted)spr(global_table_data.boosted_decal,e.x,e.y)
draw_sprite_shadow(n,e,2,self.animator.sprite_size,t)draw_sprite_rotated(n,e,self.animator.sprite_size,t)
...
self.manifest_cooldown=self.cooldown local n,e,t=(selector.position/8-self.position)/8,self.position+Vec:new(1,0),self.dmg*1for i=1,3do Tower.apply_damage(self,raycast(e,64,n),t)e.x-=1end e+=Vec:new(2,1)for i=1,3do Tower.apply_damage(self,raycast(e,64,n),t)e.y-=1end end function Tower:manifested_hale_blast()
if(self.manifest_cooldown>0)return
self.manifest_cooldown=self.cooldown local e=selector.position/8local n,t={},{e,e+Vec:new(0,1),e+Vec:new(0,-1),e+Vec:new(-1,0),e+Vec:new(1,0)}for e in all(t)do add_enemy_at_to_table(e,n,true)end spawn_particles_at(t,global_table_data.animation_data.frost)Tower.freeze_enemies(self,n)Tower.apply_damage(self,n,self.dmg\9)end function Tower:manifested_nova()self.manifest_cooldown=min(self.manifest_cooldown+9,110)self.dmg=round_to(min(self.manifest_cooldown,100)/15,2)end function Tower:manifested_torch_trap()local e=selector.position/8
...
return end self.position,grid[e.y][e.x],grid[n.y][n.x],self.enable=e,"floor","path",true end function Tower:manifested_sharp_rotation()self.dir=selector.position/8-self.position self.rot=acos(self.dir.y/sqrt(self.dir.x*self.dir.x+self.dir.y*self.dir.y))*360-180
if(self.dir.x>0)self.rot*=-1
if(self.rot<0)self.rot+=360
if(self.rot>360)self.rot-=360
end function raycast(o,i,n)
if(n==Vec:new(0,0))return
local e,t={},{}for a=1,i do local i=Vec.floor(o+n*a)add(t,i)add_enemy_at_to_table(i,e)end
if(#e>0or manifested_tower_ref)spawn_particles_at(t,global_table_data.animation_data.spark)
return e end function manifest_tower_at(n)for e in all(towers)do if e.position==n then
if(e.being_boosted)e.being_boosted=false
e.being_manifested,manifested_tower_ref,manifest_selector.dir=true,e,1if e.type=="tack"then lock_cursor,e.attack_delay,e.dmg=true,12,0elseif e.type=="sharp"then e.attack_delay/=2end end end end function unmanifest_tower()manifested_tower_ref.being_manifested=false manifest_selector.dir=-1lock_cursor=false if manifested_tower_ref.type=="tack"then local e=global_table_data.tower_templates[1]manifested_tower_ref.attack_delay=e.attack_delay manifested_tower_ref.dmg=e.damage elseif manifested_tower_ref.type=="sharp"then manifested_tower_ref.attack_delay=global_table_data.tower_templates[5].attack_delay end manifested_tower_ref.enable=true manifested_tower_ref=nil end function place_tower(e,t)local n=global_table_data.tower_templates[t or selected_menu_tower_id]if not t then
if(#towers>=64or grid[e.y][e.x]=="tower"or coins<n.cost or n.type=="floor"~=(grid[e.y][e.x]=="path"))return
coins-=n.cost end add(towers,Tower:new(e,n,direction))grid[e.y][e.x]="tower"end function refund_tower_at(e)for n in all(towers)do if n.position==e then grid[e.y][e.x]="empty"
if(n.type=="floor")grid[e.y][e.x]="path"
coins+=n.cost\1.25del(animators,n.animator)del(towers,n)end end end function draw_tower_attack_overlay(n)local e=selector.position/8palt(global_table_data.palettes.transparent_color_id,false)pal(global_table_data.palettes.attack_tile)local i=grid[e.y][e.x]=="empty"local t=Vec:new(global_table_data.map_data[loaded_map].mget_shift)if n.type=="tack"and i then nova_apply(n.radius,function(i)local n=e+i spr(mget(Vec.unpack(n+t)),Vec.unpack(n*8))end)elseif n.type=="rail"and i then draw_ray_attack_overlay(n.radius,e,t)elseif n.type=="frontal"and i then frontal_apply({n.radius,direction},function(i)local n=e+i spr(mget(Vec.unpack(n+t)),Vec.unpack(n*8))end)elseif n.type=="floor"and grid[e.y][e.x]=="path"then spr(mget(Vec.unpack(e+t)),Vec.unpack(e*8))end pal()end function draw_ray_attack_overlay(e,n,t)for i=1,e do local e=n+direction*i spr(mget(Vec.unpack(e+t)),Vec.unpack(e*8))end end function draw_line_overlay(e)local i,n=8,(e.position+Vec:new(.5,.5))*8local t=Vec.floor(e.dir*e.radius*8+n)if e.type=="clock"then i=11for t=1,16do for n in all(towers)do if n.position==Vec.floor(e.position+e.dir*t)and not n.being_manifested and n.type~="clock"then n.being_boosted=true break end end end end
if(t~=n and manifested_tower_ref)line(n.x,n.y,t.x,t.y,i)
...
del(particles,e)end function spawn_particles_at(e,n)for t in all(e)do add(particles,Particle:new(t,false,Animator:new(n,false)))end end Animator={}function Animator:new(e,n)obj={data=e.data,sprite_size=e.size or 8,animation_frame=1,frame_duration=e.ticks_per_frame,tick=0,dir=1,continuous=n}setmetatable(obj,self)self.__index=self return obj end function Animator:update()self.tick=(self.tick+1)%self.frame_duration
...
return self.animation_frame<=1end function Animator:draw(t,i)local n,e=Vec:new(t,i),self.data[self.animation_frame]
if(e.offset)n+=Vec:new(e.offset)
spr(e.sprite,Vec.unpack(n))end function Animator:get_sprite()return self.data[self.animation_frame].sprite end BorderRect={}function BorderRect:new(e,n,t,i,o)obj={position=e,size=e+n,border=t,base=i,thickness=o}setmetatable(obj,self)self.__index=self return obj end function BorderRect:draw()rectfill(self.position.x-self.thickness,self.position.y-self.thickness,self.size.x+self.thickness,self.size.y+self.thickness,self.border)rectfill(self.position.x,self.position.y,self.size.x,self.size.y,self.base)end Menu={}function Menu:new(i,o,e,n,t,a,r,l,d,f)obj={name=i,prev=o,position=Vec:new(e,n),selector=Animator:new(global_table_data.animation_data.menu_selector,true),up_arrow=Animator:new(global_table_data.animation_data.up_arrow,true),down_arrow=Animator:new(global_table_data.animation_data.down_arrow,true),content=t,content_draw=a,rect=BorderRect:new(Vec:new(e,n),Vec:new(10+5*longest_menu_str(t),38),l,r,f),text=d,pos=1,enable=false,ticks=5,max_ticks=5,dir=0}setmetatable(obj,self)self.__index=self return obj end function Menu:draw()
if(not self.enable)return
...
if(e)self.color=e
local e,t=self.width,""for o,n in pairs(split(i," "))do if#n+1<=e then t..=n.." "e-=#n+1elseif#n<=e then t..=n e-=#n else t..="\n"..n.." "e=self.width-#n+1end end self.data,e={},0local n,i="",split(t,"\n")for o,t in pairs(i)do if e<=self.max_lines then n..=t.."\n\n"e+=1else add(self.data,n)n,e=t.."\n\n",1end
if(o==#i)add(self.data,n)
...
if shop_enable then shop_loop()else game_loop()end end TextScroller.update(text_scroller)if btnp(🅾️)then if text_scroller.is_done then text_scroller.enable=false else TextScroller.skip(text_scroller)end end end function main_menu_draw_loop()map(unpack(global_table_data.splash_screens[1].mget_shift))local e,t,i=1,0,0for n,o in pairs(global_table_data.letters)do
if(e==8)e,t,i=1,18,16
local a,r=Vec:new(e*16-8+i-((n==5or n==9)and 2or 0),letters[n]+t),n==5and letter_rot or 0draw_sprite_shadow(o,a,4,16,r)draw_sprite_rotated(o,a,16,r)e+=1end if menu_enemy then Enemy.draw(menu_enemy,true)Enemy.draw(menu_enemy)end Menu.draw(get_menu"main")print_with_outline("v2.0.3",1,122,7,1)end function credits_draw_loop()map(unpack(global_table_data.splash_screens[1].mget_shift))print_with_outline("credits",47,credit_y_offsets[1],7,1)print_with_outline("project developers",25,credit_y_offsets[2],7,1)print_with_outline("jasper:\n  • game director\n  • programmer",10,credit_y_offsets[3],7,1)print_with_outline("jeren:\n  • core programmer\n  • devops",10,credit_y_offsets[4],7,1)print_with_outline("jimmy:\n  • artist\n  • sound engineer",10,credit_y_offsets[5],7,1)print_with_outline("kaoushik:\n  • programmer",10,credit_y_offsets[6],7,1)print_with_outline("external developers",25,credit_y_offsets[7],7,1)print_with_outline("thisismypassport:\n  • shrinko8 developer",10,credit_y_offsets[8],7,1)print_with_outline("jihem:\n  • created the rotation\n  sprite draw function",10,credit_y_offsets[9],7,1)print_with_outline("rgb:\n  • created the acos function",10,credit_y_offsets[10],7,1)end function map_draw_loop()local e=get_menu"map"pal(global_table_data.palettes.dark_mode)map(unpack(global_table_data.map_data[e.pos].mget_shift))pal()Menu.draw(e)print_with_outline("map select",45,1,7,1)end function game_draw_loop()local e=global_table_data.map_data[loaded_map]local n=global_table_data.tower_templates[selected_menu_tower_id]map(unpack(e.mget_shift))
...
local n=is_in_table(selector.position/8,towers,true)sell_selector.dir=n and 1or-1if n and not manifested_tower_ref then if manifest_mode then print_with_outline("❎ manifest",1,115,7,0)else print_with_outline("❎ sell",1,115,7,0)Animator.update(sell_selector)Animator.draw(sell_selector,Vec.unpack(selector.position))end else if sell_mode then Animator.update(sell_selector)Animator.draw(sell_selector,Vec.unpack(selector.position))else if not manifested_tower_ref then local i,n,t=selector.position/8,7,"❎ buy & place "..e.name if e.cost>coins then t,n="can't afford "..e.name,8elseif e.type=="floor"~=(grid[i.y][i.x]=="path")then t,n="can't place "..e.name.." here",8end print_with_outline(t,1,115,n,0)end end end end end function main_menu_loop()local n,t=global_table_data.splash_screens[1],global_table_data.enemy_templates for e=1,#letters do if letters[e]<15then letters[e]+=1end end letter_rot=(letter_rot+15)%360if pathing==nil then pathing=parse_path(n)end if not menu_enemy then local e=t[flr(rnd(#t))+1]menu_enemy=Enemy:new(n.enemy_spawn_location,e.hp,e.step_delay\2,e.sprite_index,e.type,e.damage,e.height)else update_enemy_position(menu_enemy,true)end Menu.update(get_menu"main")if btnp(❎)then Menu.invoke(get_menu"main")end Menu.move(get_menu"main")end function credits_loop()
...
foreach(enemies,kill_enemy)foreach(particles,destroy_particle)if enemies_active and#enemies==0and enemies_remaining==0then enemies_active=false sfx(global_table_data.sfx_data.round_complete)coins+=15load_wave_text()end end function print_with_outline(e,n,t,i,o)
?e,n-1,t,o
?e,n+1,t
?e,n,t-1
?e,n,t+1
?e,n,t,i
end function controls()if btnp(⬆️)then return 0,-1elseif btnp(⬇️)then return 0,1elseif btnp(⬅️)then return-1,0elseif btnp(➡️)then return 1,0end return 0,0end function increase_enemy_health(e)local n=global_table_data.freeplay_stats freeplay_mod=max(0,freeplay_rounds-max_waves)return{e.hp*(1+(n.hp-1)*((wave_round+freeplay_mod)/15)),max(e.step_delay-n.speed*freeplay_mod,n.min_step_delay),e.sprite_index,e.type,e.damage,e.height}end function is_in_table(e,i,o)for n,t in pairs(i)do if o then
if(e==t.position)return true,n
else
if(e==t)return true,n
end end end function add_enemy_at_to_table(n,t,i)for e in all(enemies)do if e.position==n then add(t,e)
if(not i)return
end end end function draw_sprite_rotated(t,o,e,i,c)local u,h,n,a,r=t%16*8,t\16*8,e\2-.5,sin(i/360),cos(i/360)for l=0,e-1do for d=0,e-1do local f,s=l-n,d-n local t=flr(f*r-s*a+n)local i=flr(f*a+s*r+n)if t>=0and t<e and i>=0and i<e then local e=sget(u+t,h+i)if e~=global_table_data.palettes.transparent_color_id or c then pset(o.x+l,o.y+d,e)end end end end end function draw_sprite_shadow(t,i,e,o,a)local n={}for e=0,15do n[e]=0end pal(n)draw_sprite_rotated(t,i+Vec:new(e,e),o,a)pal()end function parse_direction(e)
if(e.x>0)return 90
if(e.x<0)return 270
if(e.y>0)return 180
if(e.y<0)return 0
end function frontal_apply(n,d)local e,a=unpack(n)local n,t,i,o,r,l=-1,1,1,e,1,1if a.x>0then n,t,i,o=1,-1,e,1elseif a.x<0then n,t,i,o,r=-1,-1,-e,1,-1elseif a.y<0then n,t,i,o,l=-1,-1,1,-e,-1end for e=t,o,l do for t=n,i,r do
if(t~=0or e~=0)d(Vec:new(t,e))
end end end function nova_apply(e,i)for n=-e,e do for t=-e,e do
...
t+=1e+=1elseif o==0then if n[e]==","then insert_key_val(sub(n,t,e-1),i)t=e+1elseif e==#n then insert_key_val(sub(n,t),i)end end::unpack_table_continue::e+=1end return i end function insert_key_val(e,t)local i,n=split_key_value_str(e)local e if n[1]=="{"and n[-1]=="}"then e=unpack_table(sub(n,2,#n-1))elseif n=="true"then e=true elseif n=="false"then e=false elseif e=="inf"then e=32767elseif e=="nil"then e=nil else e=tonum(n)or n end if i==nil then add(t,e)else t[i]=e end end function convert_to_array_or_table(n)local e=sub(n,2,#n-1)
